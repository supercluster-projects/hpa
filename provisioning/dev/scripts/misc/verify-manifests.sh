...
for script in "${SCRIPT_DIR}"/install-*.sh "${SCRIPT_DIR}"/verify-*.sh "${SCRIPT_DIR}"/setup-bridge.sh "${SCRIPT_DIR}"/cleanup.sh \
              "${SCRIPT_DIR}/steps"/*/install-*.sh "${SCRIPT_DIR}/steps"/*/verify-*.sh; do
  [ -f "${script}" ] || continue
