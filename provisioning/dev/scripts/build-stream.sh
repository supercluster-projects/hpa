#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# build-stream.sh — Build and push stream Spin WASM image to Harbor
#
# Compiles the stream-processor Rust Spin app to WASM using `spin build`,
# then pushes the OCI artifact to Harbor via `spin registry push`. The
# resulting image is consumed by the SpinOperator SpinApp CRD at:
#   gitops-workloads/functions/overlays/dev/spins/stream.yaml
#
...
#
# Usage: ./build-stream.sh [--harbor-url <url>] [--image-tag <tag>]
#                          [--spin-source-dir <dir>]
#
...
SPIN_SOURCE_DIR="backend/spins/stream"

# Date-stamped version tag (e.g. stream:20260628-001)
...
Build and push stream Spin WASM image to Harbor.
...
Options:
  ...
  --spin-source-dir DIR   Spin source directory (default: backend/spins/stream)
...
# ---- Summary --------------------------------------------------------------
echo ""
echo "=== Stream Spin WASM Build Summary ==="
...
echo "    ${LATEST_REF}"
echo ""
echo "  Next: Restart stream SpinApp to pick up new image:"
echo "    kubectl -n hpa-workloads rollout restart spinapp stream"
echo ""
echo "======================================"

log "build-stream: completed successfully"
exit 0
